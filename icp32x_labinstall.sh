...
check_files () {
	filecnt=`stat -c "%n" app_mgmt_server_*tar.gz ibm-cloud-*tar.gz icp-docker*bin config.yaml | wc -l`

	if [ $filecnt -ne 4 ]; then
            printf "${RED}"
	    echo "Compressed product files not found in the current directory."
	    echo "Place all product files in the /opt/install_media directory and run the script again."
	    printf "${NC}"
	    exit 6
	else
...
get_ulimit() {
    ...
    if [ ${vmmax} -lt  1048575 ]; then
        printf "${RED}Current vm.max_map_count is ${vmmax}${NC}\n"
        echo "======================="
	    echo "Modify the vm.max_map_count setting and then run the script again"
        echo "If vm.max_map_count < 1048575 increase the value using the command: sysctl -w vm.max_map_count=1048575"
...
config_worker() {
    ...
    if [ $? != 0 ]; then
        ...
        ssh root@$workerIP 'chmod +x /opt/install_media/*.sh'
        ssh root@$workerIP 'yum update -y'
    fi

    #CRITICAL create k8s directories at worker node
    ssh root@$workerIP 'ls -lt /k8s 2&1>/dev/null'
    if [ $? != 0]; then
        printf "${RED}k8s directories not created...STOP!${NC}"
    else        
        printf "${GREEN}k8s directories created${NC}"
        sleep 10
    fi    
...
install_icp() {
    ...
    # 2019-08-11:  Added updated config.yaml file per ICP 3.2.0 install instructions
    if [ ! -f ./config.yaml ]; then
        cp /opt/install_media/config.yaml ./.
    else
        mv ./config.yaml ./config.yaml_original
        cp /opt/install_media/config.yaml ./.
    fi
...
install_cam () {
    ...
    printf "Extracting charts from ${camMedia}....\n"
    printf "May take up to 5 minutes\n"
    sleep 5
    tar xvf $camMedia charts
    
    chartsMedia=`ls ./charts/ibm-cloud-appmgmt*tgz`
    printf "Extracting ${chartsMedia}....\n"
    printf "May take up to 1 minutes\n"
    sleep 5
    tar xvf $chartsMedia 

    docker login mycluster.icp:8500 -u admin -p admin
    printf "Loading into docker repo....\n"
    printf "${GREEN}May take up to 30 minutes${NC}\n"
    TSTAMP=`date`
    printf "${TSTAMP}\n"
    ...
    ./ibm_cloud_pak/pak_extensions/prepare-pv.sh --size0 --releaseName ibmcloudappmgmt --local --CassandraNodes $masterIP --ZookeeperNodes $workerIP --KafkaNodes $masterIP --CouchDBNodes $workerIP --DatalayerNodes $workerIP --CassandraSize 50Gi --KafkaSize 10Gi --ZookeeperSize 1Gi --CouchDBSize 1Gi --DatalayerSize 1Gi 

    kubectl create -f ./ibm_cloud_pak/yaml

    #2019-08-11:  pre-install.sh file located in different location
    ./ibm_cloud_pak/pak_extensions/pre-install.sh --accept --advanced --releasename ibmcloudappmgmt --masterAddress $masterIP --proxyIP $masterIP --proxyFQDN $proxyFQN --clusterCAdomain mycluster.icp 

# # Gary's entry:
#pre-install.sh --accept   --advanced --releasename ibmcloudappmgmt --masterAddress  9.42.46.221 --proxyIP 9.42.78.196  --proxyFQDN icam2-w2.rtp.raleigh.ibm.com --clusterCAdomain  mycluster.icp
#==================  
    ...
    # Default post-install-setup.sh syntax
    ./ibm_cloud_pak/pak_extensions/post-install-setup.sh --releaseName ibmcloudappmgmt --namespace default --instanceName ibmcloudappmgmt --advanced
    ...
    #./ibm_cloud_pak/pak_extensions/post-install-setup.sh --releaseName ibmcloudappmgmt --namespace default --instanceName ibmcloudappmgmt --advanced --tenantID 9d3679db-fe09-45b2-a5aa-ece73137711f 
}
